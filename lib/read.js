  ...
  return maybe(cb, new Promise(resolve => {
    ...
    s.on('data', function (e) {
      ...
      if (entryPathParts.length !== dstPathParts.length && isPathChild(dstPathParts, entryPathParts)) {
        // use the subname
        let name = entryPathParts.slice(dstPathParts.length).join('/')

        // make sure subdirectories exist
        // (some hyperdrives dont explicitly list all directores)
        let end = depth ? dstPathParts.length + depth : entryPathParts.length
        for (let i = dstPathParts.length + 1; i <= end; i++) {
          let subname = entryPathParts.slice(dstPathParts.length, i).join('/')
          if (!entries[subname]) {
            entries[subname] = { type: 'directory', name: subname }
          }
        }

        // add the actual entry
        let shouldInclude = !depth || (
